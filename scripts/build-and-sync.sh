...
#
#   - @scotia/rn-network: recibe una copia en ios/NetworkContracts.xcframework
#     (la usa el podspec vía vendored_frameworks).
#   - App nativa: referenciá build/NetworkContracts.xcframework por path local
...
if [[ -d "${RN_NETWORK_IOS}" ]]; then
  rm -rf "${RN_NETWORK_IOS}/NetworkContracts.xcframework"
  cp -R build/NetworkContracts.xcframework "${RN_NETWORK_IOS}/"
  echo "    copiado → ${RN_NETWORK_IOS}/NetworkContracts.xcframework"
else
...
echo ""
echo "✅ Listo."
echo "   App nativa (SPM): referenciá $(pwd)/build/NetworkContracts.xcframework"
